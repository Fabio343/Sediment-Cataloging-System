...
from .forms import UserForm,ContactForm, amostraForm, continenteForm, paísForm, estadoForm, cidadeForm,climaForm,ambienteForm,EditProfileForm
...
def grupo(request):
    filter = amostraFilter(request.POST or None)
    context = {
        "filter": filter,
        }
    return render(request, 'amostras/grupo.html', context)
def search(request):
    user_list = User.objects.all()
    user_filter = UserFilter(request.GET, queryset=user_list)
    return render(request, 'amostras/grupo.html', {'filter': user_filter})

def filtro(request):
    amostra_list=Amostra.objects.all()
    amostra_filter=amostraFilter(request.GET,queryset=amostra_list)
    return render(request,'amostras/grupo.html',{' filter':amostra_filter})
######
...
########################################################################################################################
def autoamostra(request):
    if request.is_ajax:
        codigo = request.GET.get('term', '')
        amostras = Amostra.objects.filter(codigo__icontains=codigo)
        results = []
