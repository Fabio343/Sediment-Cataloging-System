...
require get_template_directory() . '/inc/pagemetabox.php';
...
function level_setup() {
   ...
   'flex-width' => true,
	...
	register_nav_menus( array(
		...
		'footer' => esc_html__( 'Footer Menu', 'level' ),
	) );
...
add_action( 'after_setup_theme', 'level_setup' );
 function level_search_form( $form ) {
	...
	<div><label class="screen-reader-text" for="s">' . __( 'Pesquisar Por:','level' ) . '</label>
	<input type="text" class="input-group-field" placeholder="'. __( 'Pesquisar..','level' ) .'" value="' . get_search_query() . '" name="s" id="s" />
	<input class="input-group-button button" type="submit" id="searchsubmit" value="'. esc_attr__( 'Ir','level' ) .'" />
	</div>
	</div>
	</form>';
...
function level_widgets_init() {
	register_sidebar( array(
		...
		'before_widget' => '<aside id="sidebarid %1$s" class="widget levelsidebar %2$s">',
		'after_widget'  => '</aside>',
...
register_sidebar( array(
		...
		'before_widget' => '<aside id="%1$s" class="widget naviwid %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h2 class="widget-title">',
		'after_title'   => '</h2>',
	) );	
...
function level_scripts() {
   ...
   wp_enqueue_style('level-body-font', '//fonts.googleapis.com/css?family='.str_replace(" ", "+", get_theme_mod('level_body_font', 'Open Sans') ).':100,300,400,700' );
   wp_enqueue_style('level-title-font', '//fonts.googleapis.com/css?family='.str_replace(" ", "+", get_theme_mod('level_title_font', 'Open Sans') ).':100,300,400,700' );    
	...
	if ( is_rtl() ) {
	wp_enqueue_script( 'foundation-js-rtl', get_template_directory_uri() . '/foundation/rtl/js/foundation.min.js', false, null, true);
	wp_enqueue_style( 'foundation-min-css-rtl', get_template_directory_uri() . '/foundation/rtl/css/foundation.min.css' );
	wp_enqueue_style( 'level-rtl-css', get_template_directory_uri() . '/css/rtl.css' );
	}else {
	wp_enqueue_script( 'foundation-js', get_template_directory_uri() . '/foundation/js/foundation.min.js', false, null, true);
	...
	//wp_enqueue_style( 'level-custom-css', get_template_directory_uri() . '/css/custom.css' );	
	wp_enqueue_style( 'level-custom-css-min', get_template_directory_uri() . '/css/custom.min.css' );	
